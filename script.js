...
try{
const staff = fetch(URL,{method:GET});
if(!staff.ok) {
return new Error("We have some trouble")
}
let ff = await staff.json()
}catch(e){
...
async function printFiveItems() {
  for await (const particle of gen) {
...
const motIn = countdownGenerator(5);
let nextValue = motIn.next();
